...
//using namespace std;
//Tinyweb v2, a tiny web server based on libuv
...
void set_assert(uv_stream_t *server, int status)
{
...
check_sockname(struct sockaddr* addr, const char* compare_ip, int compare_port, const char* context) 
{  
...
void tinyweb_start(uv_loop_t* loop, const char* ip, int port) {
	...
	check_sockname(&sockname, "0.0.0.0", 8080, "server socket");  
	//没有连接时，peername是无意义的  
...
void write_uv_data(uv_stream_t* client, const void* data, unsigned int len, int need_copy_data, int need_free_data) {
	uv_buf_t buf;
	uv_write_t* w;
...
char* format_http_respone(const char* status, const char* content_type, const void* content, int content_length,const char* content_location) {
	printf("调用格式化的函数！\n");
	int totalsize;
	char* respone;
	if(content_length < 0)
...
void tinyweb_on_request_get(uv_stream_t* client, const char* pathinfo, const char* query_stirng) {
...
void on_uv_read(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf) 
{
	if(nread > 0) {
		...
		const char *pending_find_str = "HTTP";
		...
		if((crln2 = strstr((const char*)membuf->data, "\r\n\r\n")) != NULL) {
			...
			*crln2 = '\0';
			...
			if(request_header[0]=='G' && request_header[1]=='E' && request_header[2]=='T') {
				...
				printf("****\n");
				temp=temp+1;
				temp=strtok(temp," ");//继续调用strtok，分解剩下的字符串
				printf("%s\n",temp);//输出分解的字符串
				printf("****\n");
				//自己添加结束
...
void tinyweb_on_connection(uv_stream_t* server, int status) {
	printf("enter on_new_connection...\n");
        uv_stream_t* server_temp = server;
	int status_temp = status;
	set_assert(server_temp, status_temp);  
}

int main()
{
	tinyweb_start(uv_default_loop(), "0.0.0.0", 8080);
