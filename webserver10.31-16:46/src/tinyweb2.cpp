...
string CTcpServer::Serial_head_userurl(string userurl, int userid)   //userid type is wrong! correct type is string!
{
	...
	Cbody_GetCDN obj_getcdn;
	string str2 = obj_getcdn.Serial_GetCDN(obj_getcdn, userurl, userid);
	str1.append(str2);
		...
		if((crln2 = strstr((char*)membuf->data, "\r\n\r\n")) != NULL) 
		{
			...
			*crln2 = '\0';
			...
			if(request_header[0]=='G' && request_header[1]=='E' && request_header[2]=='T') 
			{
				...
				pending_send_str = CTcpServer::Serial_head_userurl(str, 1);  // 1 is 
...
casereturn CTcpServer::Start_to_SerializeorParse(const string &stemp, int client)
{
	...
	switch (headstruct.cmdtype)
	{
		...
		case 11: 
		{
			...
			casedeail.clienthandler = bodytemp.iuserid;  //这里是有问题的，应该改为string型
		    break;  
		} 
		//只需要解析头部
		case 12: cout<<"CDN successfully scratches contents!";break;
		case 13: cout<<"This is a leep-Allive package sent by CDN!";break;
