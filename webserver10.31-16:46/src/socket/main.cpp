...
{  
    ...
    free(req_write);  
	////出现了结构体成员指针释放错误free(base); 
   // CTcpServer::start_read();  
}

casereturn CTcpServer::Start_to_Parse(const string &stemp, int client)
{
...
int CTcpServer::start_listener(uv_loop_t* loop, const char* ip, int port) 
{  
...
{  
	...
	CTcpServer::start_listener(loop, "0.0.0.0", server_port);  
