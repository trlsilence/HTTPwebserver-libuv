...
void on_read_client(uv_stream_t *client, ssize_t nread, const uv_buf_t* buf) 
{
	...
	if(nread>0)
	{
		...
		else
		{
			...
			//printf("nread:%d\n",nread);  //打印出收到的字符数
			//caseresult=CTcpServer::Start_to_Parse(buf->base, int client);
			uv_stream_t *sendclient;
...
casereturn Deail_CaseOne(const string &stemp, int client)  //处理命令字1
{
	...
	temp_head = obj_Serialize.Serialhead(obj_Serialize, fversion, 7.0, headstruct.cmdtype, default_StateID, default_rsptime);
...
{	
	...
	temp_head = obj_Serialize.Serialhead(obj_Serialize, fversion, 7.0, 7.0, default_StateID, default_rsptime);
	bodytemp = obj_delfile.Parse_DelFile(obj_delfile, stemp);
...
casereturn Deail_CaseEight(const string &stemp, int client)
{
	...
	temp_head = obj_Serialize.Serialhead(obj_Serialize, fversion, 7.0, 8.0, default_StateID, default_rsptime);
...
casereturn Deail_CaseNine(const string &stemp, int client)
{
	...
	temp_head = obj_Serialize.Serialhead(obj_Serialize, fversion, 7.0, 9.0, default_StateID, default_rsptime);
...
casereturn Deail_CaseTen(const string &stemp, int client)
{
	...
	temp_head = obj_Serialize.Serialhead(obj_Serialize, fversion, 7.0, 10.0, default_StateID, default_rsptime);
...
casereturn Deail_CaseEleven(const string &stemp, int client)   //提取URL和UserID,返回给用户进行重定向
{
	...
	char * id=(char*)bodytemp.url.data();
	char * url=(char*)bodytemp.userid.data();
	UserIDtoHandlerManager *usertohandler = UserIDtoHandlerManager::Instance();
