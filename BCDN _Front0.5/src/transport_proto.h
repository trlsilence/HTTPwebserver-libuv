...
class Cbody_ReturnManagerIDtoCDN : public Transport_proto_CBodyFront_ReturnManagerIDtoCDN   //命令字还是13，返回给CDN的心跳包
{
	public:
		...
		~ Cbody_ReturnManagerIDtoCDN(){}
		virtual string Serial_ReturnManagerIDtoCDN(Cbody_ReturnManagerIDtoCDN report_ip, int managerid, int cdnid);
...
class Cbody_AssignCDNRep : public Transport_proto_CBodyFront_AssignCDNRep //命令字4：指派CDN结果上报
{
	public:
		...
		~ Cbody_AssignCDNRep(){}
		//填充首部
		virtual string Serial_AssignCDNRep(Cbody_AssignCDNRep assigncdnRep, int cdnid, const string &requested_url);
...
class Cbody_FileInfo : public Transport_proto_CBodyCDN_FileInfo  //命令字10：CDN文件信息上报 
{
	public:
		Cbody_FileInfo(){}
		~ Cbody_FileInfo(){}
		virtual string Serial_FileInfo(Cbody_FileInfo file, int managerid, const string &filemessage, int cdnid);
...
class Cbody_StateInfo : public Transport_proto_CBodyCDN_StateInfo  //命令字9：CDN状态信息上报
{
	public:
		Cbody_StateInfo(){}
		~ Cbody_StateInfo(){}
		virtual string Serial_StateInfo(Cbody_StateInfo State,int maxband, int useband, int dcdnband, int maxspace,int usespace, int healthdegree, int lastkeeptime, int managerid, int cdnid);
...
class Cbody_GetCDNRsp : public Transport_proto_CBodyBack_GetCDNRsp  //命令字1：获取CDN的响应,后端没有存有该内容的CDN节点
{
	public:
		Cbody_GetCDNRsp(){}
		~ Cbody_GetCDNRsp(){}
	virtual string Serial_GetCDNRsp(Cbody_GetCDNRsp getcdnrsp,const string &requestedurl, int cdnid);
...
class Cbody_CDNFindSourceResult : public Transport_proto_CBodyBack_CDNFindSourceResult
{
	public:
