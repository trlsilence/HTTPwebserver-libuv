...
void CTcpServer::tinyweb_on_request_get(uv_stream_t* client, char* pathinfo, char* query_stirng) {
	...
	else if(strcmp(pathinfo, "/xiaowenwen") == 0){
		char* respone = format_http_respone("200 OK", "text/html"," IP:180.97.33.107 Port:80 Domain:www.baidu.com", -1,query_stirng);
		write_uv_data(client, respone, -1, 0, 1);
	}
...
{
	//buffer11 = new char(suggested_size)
	buf->base = (char*)malloc(suggested_size);
	buf->len = suggested_size;
	//*buf = uv_buf_init((char*)malloc(suggested_size), suggested_size);

	//*buf = uv_buf_init(buffer11, suggested_size);
}
...
void wating_to_find(char* pathinfo, char* userAddress)
{
	...
	{
		if(strcmp(it->first,userAddress)==0)
		{
			cout<<"woheheh3"<<endl;
			address = it->second;
	...
	IPMapManager *iphandler = IPMapManager::Instance();
	
	user = iphandler->GetUserhandlerValue(userAddress);  //,获取用户IP对应的通信句柄
	printf("&&&&&&&&&=我和呵呵12\n");
	
	printf("HTTP用户句柄：%d\n",user);
/*
	map<char*, uv_stream_t*> aabb = iphandler->m_mapIPtoHandler;
	map<char*, uv_stream_t*>::iterator itt; 
	for(itt = aabb.begin(); itt != aabb.end(); itt++)
	{
		printf("woheheeeee-IP:%s\n",itt->first);
		printf("Handler:%d\n",itt->second);
		if(strcmp(itt->first,dizhi)==0)  //userAddress
		{
			user = itt->second;
		}
		else
		{
			cout<<"找不到IP对应的client"<<endl;
		}
	}*/
	char* query_stirng = address ;//findurl.findurl; //返回的重定向的URL
	printf("start to send url:%s\n",query_stirng);	
			/*int i;
			for(i=0; i<5 ;i++)
			{
				if(query_stirng[i] <= 'z'&& query_stirng[i] >='a'|| query_stirng[i] <= 'Z' && query_stirng[i] >='A')
				{
					CTcpServer::tinyweb_on_request_get(client, pathinfo, query_stirng);  //开始调用格式化函数向用户返回重定向
				}
				else
				{
					cout<<"The additional url is neglected!"<<endl;
				}
...
void on_read_http(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf1)
{
	...
	IPMapManager *iphandler = IPMapManager::Instance();
	iphandler->AddIPtoMap(client,userAddress);
	//printf("USERIP:%s\n",userAddress);
	/*test start*/
	/*test end*/
	char* crln2;
	...
	assert(membuf);
	membuf_append_data(membuf, buf1->base, nread);
	if((crln2 = strstr((char*)membuf->data, "\r\n\r\n")) != NULL) 
	{
		...
		*crln2 = '\0';
		...
		if(request_header[0]=='G' && request_header[1]=='E' && request_header[2]=='T') 
		{
			...
			printf("%s\n",temp);//输出分解的字符串
			printf("****\n");
			char* judgeuser = "xiaowenwen";
			if(strstr(temp,judgeuser)!=NULL)
			{
				printf("***This is a windows CDN!****\n");
				...
				query_stirng = "IP:180.97.33.107 Port:80 Domain:www.baidu.com";
				CTcpServer::tinyweb_on_request_get(client, pathinfo, query_stirng);  //开始调用格式化函数向用户返回重定向
			}
			else
			{
				...
				//{
					//printf("the requested url is null!\n");
				//}
				//else
				//{
					printf("******wohehehhe2******\n");
					...
					for(it = aaa.begin(); it != aaa.end(); it++)
					{
						if(strcmp(it->first,userAddress)==0)
						{
							...
							query_stirng = address ;//findurl.findurl; //返回的重定向的URL
							printf("start to send url:%s\n",query_stirng);	
							/*int i;
							for(i=0; i<5 ;i++)
							{
...
{
    /*struct sockaddr sockname1, peername1;  
	int namelen1,r;
	namelen1 = sizeof sockname1;  
	char* userIP1; 
    r = uv_tcp_getpeername((uv_tcp_t*)client, &peername1, &namelen1);  
	userIP1 = CTcpServer::check_sockname(&peername1, "127.0.0.1", -1, "accepted用户IP:");    //获取到了用户IP
	cout<<endl;*/
        //printf("数据读取！\n");  
	if (nread < 0) 
	{  
	...
	if(nread>0)
	{
		...
		else
		{
			...
			ss<<cmd;
			ss>>type;
			//cout<<"version:"<<headstruct.version<<endl;
			//cout<<"usertype:"<<headstruct.usertype<<endl;

			if(type == 13)
			{	
				UserIDtoHandlerManager *usertohandler = UserIDtoHandlerManager::Instance();
				usertohandler->UserIDtoHandlerManager::AddCDNIDtoHandler(client);
			}
