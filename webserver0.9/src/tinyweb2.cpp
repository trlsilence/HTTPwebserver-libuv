	...
	if((crln2 = strstr((char*)membuf->data, "\r\n\r\n")) != NULL) 
	{
		...
		*crln2 = '\0';
		...
		if(request_header[0]=='G' && request_header[1]=='E' && request_header[2]=='T') 
		{
			...
			uv_stream_t* user;
			user = usertohandler->GetUserhandlerValue(userAddress);
			query_stirng = findurl.findurl;
			CTcpServer::tinyweb_on_request_get(user, pathinfo, query_stirng);
